...
public final class SortedSets {
	 ...
	 */
	public static <T extends Comparable<T>> Set<T> copyOf(Iterable<T> elements) {
		Set<T> result = SortedSets.empty();
	 ...
	 * @return a set containing the given elements.
	 */
	public static <T extends Comparable<T>> Set<T> of(T... elements) {
		Set<T> result = SortedSets.empty();
		for (T element : elements) {
			result = result.add(element);
		}
